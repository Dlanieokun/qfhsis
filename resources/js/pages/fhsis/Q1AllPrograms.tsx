...
const SectionA = ({ familyPlanning }: { familyPlanning?: FamilyPlanningData }) => {
  ...
  // Quarter), New Acceptors (Previous Quarter), Other Acceptors (Present Quarter),
  // Drop-outs (Present Quarter), Current Users (End of the Quarter). "New Acceptors
  // (Present Quarter)" has no backing data yet, so it's left undefined/blank.
      ...
      <table className="w-full border-collapse text-xs">
        <tbody>
          <tr className="bg-gray-100">
            ...
            <Th colSpan={4}>Current Users (Beginning of the Quarter)</Th>
            <Th colSpan={8}>Acceptors</Th>
            <Th colSpan={4}>Drop-outs (Present Quarter)</Th>
            <Th colSpan={4}>Current User (End of the Quarter)</Th>
            <Th colSpan={4}>New Acceptors (Present Quarter)</Th>
          </tr>
          <tr className="bg-gray-50">
            <Th colSpan={4}>Age Group / TOTAL</Th>
            <Th colSpan={4}>New Acceptors (Previous Quarter)</Th>
            <Th colSpan={4}>Other Acceptors (Present Quarter)</Th>
            <Th colSpan={4}>Age Group / TOTAL</Th>
            <Th colSpan={4}>Age Group / TOTAL</Th>
            <Th colSpan={4}>Age Group / TOTAL</Th>
          </tr>
    ...
    <div className="mb-6">
      <table className="w-full border-collapse text-xs">
        <tbody>
          ...
          <tr>
            <Td className="pl-4">The total number of identified adult (20-59 years old) hypertensives in the current quarter</Td>
            <SexInputsFromData data={nonCommunicableDisease?.cvd2059} /><InputCell />
            <Td className="pl-4">The total number of identified adult (20-59 years old) with Type II Diabetes in the current quarter</Td>
...
// ─── MAIN COMPONENT ───────────────────────────────────────────────────────────
export default function Q1AllPrograms({
  ...
  // Static Quarter Array Mapping (Q1: Jan-Mar, Q2: Apr-Jun, Q3: Jul-Sep, Q4: Oct-Dec)
  const quarters = [
    "1st Quarter (Jan - Mar)",
    "2nd Quarter (Apr - Jun)",
    "3rd Quarter (Jul - Sep)",
  ...
  // ─── Filter -> API Query Builder (shared: quarter/year/region/province/municipality/barangay) ───
  const buildReportQuery = () => {
    const params = new URLSearchParams();

    const quarterNumber = selectedQuarter
      ? quarters.indexOf(selectedQuarter) + 1
      : Math.floor(new Date().getMonth() / 3) + 1;
    params.set('quarter', String(quarterNumber));
  ...
  const DynamicFormHeader = () => (
    <div className="mb-6 p-4 bg-slate-50 rounded-lg border border-slate-200 grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
      ...
      <div className="space-y-2">
        <label className="block font-semibold text-gray-700">Reporting Quarter</label>
        <select 
      ...
      <div className="md:col-span-3 flex flex-wrap items-center gap-3 pt-1 border-t border-slate-200 mt-1">
        ...
        {!fpError && !mcError && !ccError && !ohError && !ncdError && !ehError && !idError &&
          ...
          <span className="text-gray-500 text-xs">
            Showing data for {selectedQuarter || quarters[Math.floor(new Date().getMonth() / 3)]} {selectedYear}
  ...
  return (
    <div className="bg-white shadow-sm rounded-lg border border-gray-200 p-4">
      ...
      <div className="mb-4 flex flex-wrap justify-between items-center gap-2">
        <div>
          <h2 className="text-xl font-bold text-gray-800">Q1: All Programs</h2>
          <p className="text-xs text-gray-500">FHSIS Quarterly Report Form</p>
        </div>
        <div className="flex gap-2">
          <a
            href="/qfhsis/public/fhsis/reports/export-q1-all"
